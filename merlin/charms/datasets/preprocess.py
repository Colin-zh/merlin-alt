...
class BoolPreprocessor(BaseEstimator, TransformerMixin):
    ...
    def fit(self, X, y=None):
        self.ordinal_encoders = OrdinalEncoder(
            handle_unknown='use_encoded_value', unknown_value=np.nan)
        self.ordinal_encoders.fit(X[self.bool_features])
        return self
    
    def transform(self, X, y=None):
        return self.ordinal_encoders.transform(X[self.bool_features]).astype(np.float32)
...
class OneHotPreprocessor(BaseEstimator, TransformerMixin):
    ...
    def fit(self, X, y=None):
        ...
        self.labels = {}
        for f in self.onehot_features:
            self.onehot_encoders[f].fit(self._fillna(X[[f]]))
            self.labels[f] = self.onehot_encoders[f].categories_
        # Update feature names
        self._feature_names = []
        for k, v in self.labels.items():
            self._feature_names.extend([f"{k}_{label}" for label in v])
    
    def transform(self, X, y=None):
        onehot_encoded = []
        for f in self.onehot_features:
            onehot_encoded.append(
                self.onehot_encoders[f].transform(self._fillna(X[[f]]))
            )
        return np.hstack(onehot_encoded).astype(np.float32)
