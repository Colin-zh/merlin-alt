"""
Modified from pytorch_tabular, licensed under MIT.
Original source: https://github.com/pytorch-tabular/pytorch_tabular
Modifications made: Adapted for use in this project.

Original copyright: Copyright (c) Manu Joseph V
Modified work copyright: Copyright (c) Colin-zh

See LICENSE for full license terms.
"""

import warnings
from abc import ABCMeta, abstractmethod
from typing import Any, Callable, Dict, List, Optional, Tuple, Type, Union

import numpy as np
import pandas as pd

import torch
import torch.nn as nn
from torch import Tensor
from torch.optim import Optimizer
from torch.utils.data import DataLoader

from .common.heads import blocks
from .common.utils.nn_utils import reset_all_weights


class BaseModel(nn.Module, metaclass=ABCMeta):
    def __init__(
        self,
        config,
        **kwargs
    ):
        """Base class for all models.
        
        Args:
            config: Configuration object containing model parameters.
            **kwargs: Additional keyword arguments.
        """
        super().__init__()
        self.hparams = config
        self._build_model()
        self._setup_loss()
    
    @abstractmethod
    def _build_model(self):
        """Build the model architecture."""
        pass

    @property
    def backbone(self):
        raise NotImplementedError("backbone property needs to be implemented by inheriting classes")

    @property
    def embedding_layer(self):
        raise NotImplementedError("embedding_layer property needs to be implemented by inheriting classes")

    @property
    def head(self):
        raise NotImplementedError("head property needs to be implemented by inheriting classes")

    def _get_head_from_config(self):
        """Get the head module based on the configuration."""
        _head_callable = getattr(blocks, self.hparams.head)
        return _head_callable(
            input_dim = self.backbone.output_dim,
            output_dim = self.hparams.output_dim,
            config = _head_callable._config_template(**self.hparams.head_config)
        )

    def _setup_loss(self):
        try:
            self.loss = getattr(nn, self.hparams.loss)()
        except AttributeError as e:
            print(f"{self.hparams.loss} is not a valid loss defined in the torch.nn module")
            raise e
    
    def reset_weights(self):
        """Reset model weights to their initial state."""
        reset_all_weights(self.backbone)
        reset_all_weights(self.head)
        reset_all_weights(self.embedding_layer)
    
    def forward(self, x: Dict[str, Tensor]) -> Tensor:
        """Forward pass through the model.

        Args:
            x (Dict[str, Tensor]): The input of the model with 'continuous' and 'categorical' keys.
        
        """
        x = self.embed_input(x)
        x = self.compute_backbone(x)
        return self.compute_head(x)
    
    @torch.no_grad()
    def predict(self, x: Dict[str, Tensor], ret_model_output: bool = False) -> Union[Tensor, Tuple[Tensor, Dict]]:
        """Predicts the output of the model.
        
        Args:
            x (Dict): The input of the model with 'continuous' and 'categorical' keys

            ret_model_output (bool): If True, the method returns the output of the model

        Returns:
            The output of the model

        """
        ret_value = self.forward(x)
        if ret_model_output:
            return ret_value.get("logits"), ret_value
        return ret_value.get("logits")
    
    def compute_loss(self, output: Dict[str, Any], y: Tensor) -> Tensor:
        """Calculates the loss of the model.
        
        Args:
            output (Dict[str, Any]): The output of the model.
            y (Tensor): The ground truth labels.
        
        Returns:
            Tensor: The calculated loss.
        
        """
        y_hat = output["logits"]
        reg_terms = [k for k, v in output.items() if "reg_term" in k]
        reg_loss = 0.0
        for t in reg_terms:
            if output[t] != 0:
                reg_loss += output[t]
        if self.hparams.task == "regression":
            computed_loss = reg_loss
            for i in range(self.hparams.output_dim):
                _loss = self.loss(y_hat[:, i], y[:, i])
                computed_loss += _loss
        else:
            # TODO loss fails with batch size of 1 for classification
            computed_loss = self.loss(y_hat.squeeze(), y.squeeze()) + reg_loss
        return computed_loss
    
    def extract_embedding(self):
        """Extracts the embedding layer of the model."""
        pass
    
    def feature_importance(self) -> pd.DataFrame:
        """Calculate feature importance.

        Returns:
            pd.DataFrame: DataFrame containing feature importance scores.
        """
        if hasattr(self.backbone, "feature_importance_"):
            imp = self.backbone.feature_importance_
            n_feat = len(self.hparams.categorical_cols) + len(self.hparams.continuous_cols)
            if self.hparams.categorical_dim > 0:
                if imp.shape[0] != n_feat:
                    # Combining Cat Embedded Dimensions to a single one by averaging
                    wt = []
                    norm = []
                    ft_idx = 0
                    for _, embed_dim in self.hparams.embedding_dims:
                        wt.extend([ft_idx] * embed_dim)
                        norm.append(embed_dim)
                        ft_idx += 1
                    for _ in self.hparams.continuous_cols:
                        wt.extend([ft_idx])
                        norm.append(1)
                        ft_idx += 1
                    imp = np.bincount(wt, weights=imp) / np.array(norm)
                else:
                    # For models like FT-Transformer, we dont need to do anything
                    # It takes categorical and continuous as individual 2-D features
                    pass
            importance_df = pd.DataFrame({
                "Features": self.hparams.categorical_cols + self.hparams.continuous_cols,
                "Importance": imp
            }).sort_values(by="Importance", ascending=False).reset_index(drop=True)
            return importance_df
        else:
            raise NotImplementedError("Feature importance unavailable for this model.")

    def data_aware_intialization(self, dataloader: DataLoader):
        """Perform data-aware initialization using a dataloader.

        Args:
            dataloader (DataLoader): DataLoader providing the data for initialization.
        """
        pass

    def embed_input(self, x: Dict[str, Tensor]) -> Tensor:
        return self.embedding_layer(x)
    
    def compute_backbone(self, x: Dict[str, Tensor]) -> Tensor:
        return self.backbone(x)
    
    def compute_head(self, backbone_features: Tensor) -> Dict[str, Any]:
        """Computes the head of the model.
        
        Args:
            backbone_features (Tensor): Features from the backbone network.
        
        Returns:
            Dict[str, Any]: Output dictionary containing logits and other information.
        """
        logits = self.head(backbone_features)
        return self.pack_output(logits, backbone_features)
    
    def pack_output(self, logits: Tensor, backbone_features: Tensor) -> Dict[str, Any]:
        """Packs the output of the model into a dictionary.
        
        Args:
            logits (Tensor): The raw output from the head of the model.
            backbone_features (Tensor): Features from the backbone network.
        
        Returns:
            Dict[str, Any]: Output dictionary containing logits and other information.
        """
        # If self.head is the Identity function it means that we cannot extract backbone features,
        # because the model cannot be divide in backbone and head (i.e. TabNet)
        if type(self.head) is nn.Identity:
            return {"logits": logits}
        return {"logits": logits, "backbone_features": backbone_features}
